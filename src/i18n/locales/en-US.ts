export default {
  // Common
  welcome: "Hello, welcome back!",
  viewMore: "View more",
  home: "Home",
  logout: "Logout",
  appName: "Schedule",
  appSubtitle: "Manager",

  // Login Hero - Verses
  loginHero: {
    verses: [
      {
        text: "For even the Son of Man did not come to be served, but to serve, and to give his life as a ransom for many.",
        reference: "Mark 10:45"
      },
      {
        text: "You were called to freedom. But do not use your freedom to indulge the flesh; rather, serve one another humbly in love.",
        reference: "Galatians 5:13"
      },
      {
        text: "Each of you should use whatever gift you have received to serve others, as faithful stewards of God's grace in its various forms.",
        reference: "1 Peter 4:10"
      },
      {
        text: "Whoever wants to become great among you must be your servant.",
        reference: "Matthew 20:26"
      },
      {
        text: "Whatever you do, work at it with all your heart, as working for the Lord, not for human masters.",
        reference: "Colossians 3:23"
      },
      {
        text: "As for me and my household, we will serve the Lord.",
        reference: "Joshua 24:15"
      },
      {
        text: "Serve wholeheartedly, as if you were serving the Lord, not people.",
        reference: "Ephesians 6:7"
      },
      {
        text: "Whoever serves me must follow me; and where I am, my servant also will be.",
        reference: "John 12:26"
      }
    ],
    bibleVersion: "NIV",
    logoAlt: "Church of the Nations Logo",
    preparingEnvironment: "Preparing your environment..."
  },

  // Login Page
  login: {
    title: "Schedule Manager",
    subtitle: "Enter your credentials to access the system",
    email: "Email",
    password: "Password",
    forgotPassword: "Forgot my password",
    rememberMe: "Stay logged in",
    submit: "Sign in",
    submitting: "Signing in...",
    or: "or",
    continueWithGoogle: "Continue with Google",
    noAccount: "Don't have an account?",
    requestAccess: "Request access",
    validation: {
      emailRequired: "Email is required",
      invalidEmail: "Invalid email",
      passwordRequired: "Password is required",
      passwordMin: "Password must be at least 6 characters"
    },
    forgotPasswordDialog: {
      title: "Recover password",
      description: "Enter your email and we'll send you a link to reset your password.",
      emailLabel: "Email",
      cancel: "Cancel",
      submit: "Send",
      submitting: "Sending...",
      success: "Recovery email sent successfully! Check your inbox.",
      error: "Error sending recovery email"
    }
  },

  // Sidebar Menu
  menu: {
    inicio: "Home",
    relatorios: "Reports",
    usuarios: "Users",
    ministerios: "Ministries",
    voluntarios: "Volunteers",
    familias: "Families",
    agenda: "Schedule",
    eventosFixos: "Fixed Events",
    configuracoes: "Settings"
  },

  // Page Titles
  pageTitle: {
    home: "Home",
    reports: "Analytics and Statistics",
    users: "Manage Users",
    ministries: "Manage Ministries",
    volunteers: "Manage Volunteers",
    families: "Manage Families",
    schedule: "Event Schedule",
    fixedEvents: "Fixed Events",
    settings: "Settings"
  },

  // Breadcrumbs
  breadcrumb: {
    inicio: "Home",
    relatorios: "Reports",
    usuarios: "Users",
    ministerios: "Ministries",
    voluntarios: "Volunteers",
    familias: "Families",
    agenda: "Schedule",
    eventosFixos: "Fixed Events",
    configuracoes: "Settings"
  },

  // Dashboard
  dashboard: {
    overview: "Overview",
    upcomingEvents: "Upcoming Events",
    ministries: "Ministries",
    allEvents: "All Upcoming Events",
    allMinistries: "All Ministries"
  },

  // Stats Cards
  stats: {
    volunteers: "Volunteers",
    ministries: "Ministries",
    events: "Events",
    families: "Families",
    users: "Users",
    volunteer_count: "{{count}} volunteer",
    volunteer_count_plural: "{{count}} volunteers"
  },

  // Reports
  reports: {
    title: "Analytics and Statistics",
    volunteerGrowth: "Volunteer Growth",
    ministryDistribution: "Ministry Distribution",
    monthlyActivities: "Monthly Activities",
    newVolunteers: "New Volunteers",
    totalVolunteers: "Total Volunteers",
    activities: "activities",
    services: "Services",
    meetings: "Meetings",
    specialEvents: "Special Events",
    volunteers: "volunteers"
  },

  // User Roles
  roles: {
    admin: "Administrator",
    coordinator: "Coordinator",
    volunteer: "Volunteer"
  },

  // Common Actions
  actions: {
    save: "Save",
    cancel: "Cancel",
    edit: "Edit",
    delete: "Delete",
    add: "Add",
    search: "Search",
    filter: "Filter",
    export: "Export",
    close: "Close"
  },

  // Settings
  settings: {
    title: "Settings",
    theme: {
      title: "Theme",
      description: "Toggle between light and dark theme"
    },
    language: {
      title: "Language",
      description: "Select system language"
    }
  }
};